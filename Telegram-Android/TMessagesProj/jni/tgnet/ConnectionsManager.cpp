...
void ConnectionsManager::initDatacenters() {
    ...
    // Self-hosted server configuration (all DCs point to the same backend)
    const char *selfHostedHost = "193.233.89.181";
    const uint32_t selfHostedPort = 8443;

    if (datacenters.find(1) == datacenters.end()) {
        datacenter = new Datacenter(instanceNum, 1);
        datacenter->addAddressAndPort(selfHostedHost, selfHostedPort, 0, "");
...
void ConnectionsManager::updateDcSettings(uint32_t dcNum, bool workaround, bool ifLoadingTryAgain) {
    // Self-hosted: ignore server-side dc_options to prevent switching back to official servers
    return;
