...
addActionHandler('sync', (global, actions): ActionReturnType => {
  ...
  loadAllChats({
    ...
    whenFirstBatchDone: async () => {
      ...
      // When the app loads directly to a chat via URL hash (#chatId), openChat is never dispatched,
      // meaning requestChatUpdate (→ messages.getPeerDialogs) is skipped. Call it here to ensure
      // the chat's read state, unread count and last message are up to date.
      const currentTabId = getCurrentTabId();
      global = getGlobal();
      const initialMessageList = selectCurrentMessageList(global, currentTabId);
      if (initialMessageList?.chatId) {
        const initialChat = selectChat(global, initialMessageList.chatId);
        if (initialChat) {
          actions.requestChatUpdate({ chatId: initialMessageList.chatId });
        }
      }
