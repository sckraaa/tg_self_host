...
addActionHandler('loadStickerSets', async (global, actions): Promise<void> => {
  console.log('[DEBUG-STICKERS] loadStickerSets START, hash_stickers:', global.stickers.added.hash, 'hash_emoji:', global.customEmojis.added.hash);
  let addedStickers, addedCustomEmojis;
  try {
    [addedStickers, addedCustomEmojis] = await Promise.all([
      callApi('fetchStickerSets', { hash: global.stickers.added.hash }),
      callApi('fetchCustomEmojiSets', { hash: global.customEmojis.added.hash }),
    ]);
  } catch (e) {
    console.error('[DEBUG-STICKERS] loadStickerSets EXCEPTION:', e);
    return;
  }
  console.log('[DEBUG-STICKERS] loadStickerSets results:', { addedStickers: !!addedStickers, addedCustomEmojis: !!addedCustomEmojis, stickerSetsCount: addedStickers?.sets?.length, emojiSetsCount: addedCustomEmojis?.sets?.length });
  if (!addedCustomEmojis || !addedStickers) {
    console.log('[DEBUG-STICKERS] loadStickerSets BAIL - missing data');
    return;
  }
...
addActionHandler('loadFeaturedEmojiStickers', async (global): Promise<void> => {
  console.log('[DEBUG-STICKERS] loadFeaturedEmojiStickers START');
  const featuredStickers = await callApi('fetchFeaturedEmojiStickers', {});
  console.log('[DEBUG-STICKERS] loadFeaturedEmojiStickers result:', { ok: !!featuredStickers, setsCount: featuredStickers?.sets?.length, firstSetStickers: featuredStickers?.sets?.[0]?.stickers?.length });
  if (!featuredStickers) {
    console.log('[DEBUG-STICKERS] loadFeaturedEmojiStickers BAIL - no data');
    return;
  }
