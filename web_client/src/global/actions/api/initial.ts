...
addActionHandler('initApi', (global, actions): ActionReturnType => {
  ...
  const serverConfig = buildApiServerConfig(sharedSettings);
  const serverConfigScope = buildServerConfigScope(sharedSettings);

  const hasTestParam = window.location.search.includes('test') || initialLocationHash?.tgWebAuthTest === '1';
  const isCustomServer = serverConfig.profileId === 'custom';
  writeCurrentServerConfigScope(serverConfigScope);
  ...
  void initApi(actions.apiUpdate, {
    ...
    sessionData: loadStoredSession(serverConfigScope),
    serverConfig,
    serverConfigScope,
    ...
    shouldDebugExportedSenders,
    langCode: language,
    isTestServerRequested: !isCustomServer && hasTestParam,
    accountIds,
...
addActionHandler('signOut', async (global, actions, payload): Promise<void> => {
  ...
  console.log('[LOGOUT-DEBUG] signOut action started', { forceInitApi: payload?.forceInitApi });
  ...
  try {
    ...
    await unsubscribe();
    // eslint-disable-next-line no-console
    console.log('[LOGOUT-DEBUG] calling callApi destroy...');
    await Promise.race([callApi('destroy'), pause(3000)]);
    // eslint-disable-next-line no-console
    console.log('[LOGOUT-DEBUG] callApi destroy completed');
    await forceWebsync(false);
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error('[LOGOUT-DEBUG] error during destroy:', err);
  }

  // Temporarily disable stricterdom during reset/reinit to avoid mutation spam
  // (the full UI re-renders from chat screen to auth screen)
  if (STRICTERDOM_ENABLED) {
    disableStrict();
  }

  // eslint-disable-next-line no-console
  console.log('[LOGOUT-DEBUG] calling actions.reset()');
  actions.reset();

  if (payload?.forceInitApi) {
    // eslint-disable-next-line no-console
    console.log('[LOGOUT-DEBUG] calling actions.initApi()');
    actions.initApi();
  }
...
addActionHandler('reset', (global, actions): ActionReturnType => {
  // eslint-disable-next-line no-console
  console.log('[LOGOUT-DEBUG] reset action started');
