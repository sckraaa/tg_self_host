...
import { buildServerConfigScope } from '../util/mtprotoServer';
...
export async function loadCache(initialState: GlobalState): Promise<GlobalState | undefined> {
  ...
  if (cache.passcode.hasPasscode || hasStoredSession(buildServerConfigScope(cache.sharedState.settings))) {
    setupCaching();
...
function unsafeMigrateCache(cached: GlobalState, initialState: GlobalState) {
  ...
  if (!cached.sharedState) {
    ...
    cached.sharedState.settings = {
      ...
      mtprotoServerProfile: initialState.sharedState.settings.mtprotoServerProfile,
      mtprotoCustomServerHostPattern: initialState.sharedState.settings.mtprotoCustomServerHostPattern,
      mtprotoCustomServerPort: initialState.sharedState.settings.mtprotoCustomServerPort,
      mtprotoCustomServerDefaultDcId: initialState.sharedState.settings.mtprotoCustomServerDefaultDcId,
    };
  }
