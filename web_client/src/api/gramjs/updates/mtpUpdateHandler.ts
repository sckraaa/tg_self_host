  ...
  if (update instanceof UpdateServerTimeOffset) {
    ...
    if (update instanceof GramJs.UpdateShortChatMessage) {
      ...
      if (mtpMessage instanceof GramJs.MessageService) {
        // eslint-disable-next-line no-console
        console.log('[SVC-DEBUG] updateNewMessage received MessageService:', {
          id: mtpMessage.id,
          action: mtpMessage.action?.className,
          chatId: message?.chatId,
          hasAction: !!message?.content?.action,
          actionType: message?.content?.action?.type,
        });
      } else if (message?.content?.action) {
        // eslint-disable-next-line no-console
        console.warn('[SVC-DEBUG] Regular Message has action in content (unexpected):', {
          id: mtpMessage.id,
          className: mtpMessage.className,
          actionType: message.content.action.type,
        });
      }
    ...
    if (update.message instanceof GramJs.MessageService) {
      ...
      } else if (action instanceof GramJs.MessageActionChatCreate) {
        // When we receive a chatCreate service message, the current user was added to
        // a newly created group. Trigger updateChatJoin so the chat appears in dialog list.
        if (update._entities && update._entities.some((e): e is GramJs.User => (
          e instanceof GramJs.User && Boolean(e.self)
        ))) {
          sendApiUpdate({
            '@type': 'updateChatJoin',
            id: message.chatId,
