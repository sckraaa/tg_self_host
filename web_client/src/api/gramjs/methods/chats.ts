...
export async function fetchChats({
  ...
  const messages = (resultPinned ? resultPinned.messages : [])
    ...
    .map((mtpMsg) => {
      // [DEBUG] Log service message detection in getDialogs
      if (mtpMsg instanceof GramJs.MessageService) {
        // eslint-disable-next-line no-console
        console.log('[SVC-DEBUG] getDialogs returned MessageService:', {
          id: mtpMsg.id, action: mtpMsg.action?.className,
        });
      } else if (mtpMsg instanceof GramJs.Message) {
        // eslint-disable-next-line no-console
        console.log('[SVC-DEBUG] getDialogs returned Message:', {
          id: mtpMsg.id, text: mtpMsg.message?.slice(0, 50),
        });
      }
      return buildApiMessage(mtpMsg);
    })
    .filter(Boolean);
...
export async function requestChatUpdate({
  ...
  // [DEBUG] Log service message detection in getPeerDialogs
  if (result.messages[0] instanceof GramJs.MessageService) {
    // eslint-disable-next-line no-console
    console.log('[SVC-DEBUG] getPeerDialogs returned MessageService:', {
      id: result.messages[0].id, action: result.messages[0].action?.className,
    });
  } else if (result.messages[0] instanceof GramJs.Message) {
    // eslint-disable-next-line no-console
    console.log('[SVC-DEBUG] getPeerDialogs returned Message:', {
      id: result.messages[0].id, text: (result.messages[0] as GramJs.Message).message?.slice(0, 50),
    });
  }
