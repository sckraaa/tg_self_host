...
import { setServerConfig } from '../../../lib/gramjs/Utils';
...
export async function init(initialArgs: ApiInitialArgs, onConnected?: NoneToVoidFunction) {
  ...
  const {
    ...
    hasPasskeySupport, serverConfig, serverConfigScope,
  } = initialArgs;

  setServerConfig(serverConfig);

  const session = new sessions.CallbackSession(sessionData, onSessionUpdate, serverConfigScope);
  ...
  client = new TelegramClient(
    ...
    process.env.TELEGRAM_API_HASH,
    {
      ...
      useWSS: process.env.TELEGRAM_CUSTOM_PROTOCOL !== 'ws',
...
export async function destroy(noLogOut = false, noClearLocalDb = false) {
  // eslint-disable-next-line no-console
  console.log('[LOGOUT-DEBUG] destroy() called', { noLogOut, noClearLocalDb, isConnected: client.isConnected() });
  if (!noLogOut && client.isConnected()) {
    try {
      // eslint-disable-next-line no-console
      console.log('[LOGOUT-DEBUG] sending auth.LogOut...');
      await Promise.race([
        invokeRequest(new GramJs.auth.LogOut()),
        pause(LOG_OUT_TIMEOUT),
      ]);
      // eslint-disable-next-line no-console
      console.log('[LOGOUT-DEBUG] auth.LogOut response received');
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error('[LOGOUT-DEBUG] auth.LogOut error:', err);
    }
  }
  ...
  // eslint-disable-next-line no-console
  console.log('[LOGOUT-DEBUG] calling client.destroy()');
  client.destroy();
  // eslint-disable-next-line no-console
  console.log('[LOGOUT-DEBUG] client.destroy() done');
}
