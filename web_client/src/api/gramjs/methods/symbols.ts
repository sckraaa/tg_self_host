...
export async function fetchCustomEmojiSets({ hash }: { hash?: string }) {
  console.log('[DEBUG-FETCH] fetchCustomEmojiSets START, hash:', hash);
  ...
  console.log('[DEBUG-FETCH] fetchCustomEmojiSets response:', allStickers?.className, 'sets:', allStickers?.sets?.length);

  if (!allStickers || allStickers instanceof GramJs.messages.AllStickersNotModified) {
    console.log('[DEBUG-FETCH] fetchCustomEmojiSets => undefined (NotModified or null)');
    return undefined;
  }
...
export async function fetchStickerSets({ hash }: { hash?: string }) {
  console.log('[DEBUG-FETCH] fetchStickerSets START, hash:', hash);
  const allStickers = await invokeRequest(new GramJs.messages.GetAllStickers({
    hash: hash ? BigInt(hash) : DEFAULT_PRIMITIVES.BIGINT,
  }));
  console.log('[DEBUG-FETCH] fetchStickerSets response:', allStickers?.className, 'sets:', allStickers?.sets?.length);

  if (!allStickers || allStickers instanceof GramJs.messages.AllStickersNotModified) {
    console.log('[DEBUG-FETCH] fetchStickerSets => undefined (NotModified or null)');
    return undefined;
  }
