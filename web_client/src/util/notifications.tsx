...
export async function unsubscribe() {
  ...
  try {
    // Timeout to prevent hanging if no service worker is registered
    // (navigator.serviceWorker.ready never resolves without a registered SW)
...
export async function notifyAboutMessage({
  ...
  if (!checkIfShouldNotify(chat, message)) {
    // Even when the window is focused, play sound if user is viewing a different chat
    if (document.hasFocus() && !IS_TOUCH_ENV) {
      const { chatId, type } = selectCurrentMessageList(global) || {};
      const isInDifferentChat = chatId !== chat.id || type !== 'thread';
      if (isInDifferentChat && !isSilent && !message.isSilent && !isReaction && !IS_TAURI) {
