...
import { buildServerConfigScope, writeCurrentServerConfigScope } from './mtprotoServer';
...
export async function initGlobal(force: boolean = false, prevGlobal?: GlobalState) {
  ...
  writeCurrentServerConfigScope(buildServerConfigScope(global.sharedState.settings));
  setGlobal(global);
}
