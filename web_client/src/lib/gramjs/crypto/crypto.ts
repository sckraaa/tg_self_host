...
type AesConstructor = new (key: Buffer) => {
  encrypt(block: Buffer): Uint32Array;
};

function resolveAes() {
  const namespace = AESModule as unknown as {
    default?: AesConstructor;
  };

  const candidate = (typeof AESModule === 'function'
    ? AESModule
    : namespace.default) as unknown;

  if (typeof candidate !== 'function') {
    throw new Error('@cryptography/aes default export not found');
  }

  return candidate as AesConstructor;
}

const AES = resolveAes();
...
class CTR {
  ...
  private _aes: InstanceType<typeof AES>;
