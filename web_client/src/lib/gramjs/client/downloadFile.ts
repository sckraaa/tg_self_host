    ...
    promises.push((async (offsetMemo: number, limitMemo: number, senderIndexMemo: number) => {
      while (true) {
        ...
        try {
          ...
          // eslint-disable-next-line no-console
          console.debug(`[GETFILE-REQ] offset=${offsetMemo} limit=${limitMemo} dcId=${dcId}`);
          ...
          isDone2 = true;
          // eslint-disable-next-line no-console
          console.debug(`[GETFILE-RESP] offset=${offsetMemo} got=${result.bytes.length} bytes (expected limit=${limitMemo})`);
          if (progressCallback) {
            if (progressCallback.isCanceled) {
