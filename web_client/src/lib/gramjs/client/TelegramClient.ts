...
class TelegramClient {
  ...
  _downloadPhoto(media: Api.MessageMediaPhoto | Api.TypePhoto, args: DownloadMediaParams) {
    ...
    if (!(photo instanceof Api.Photo)) {
      // eslint-disable-next-line no-console
      console.warn('[DLPHOTO] media is not Api.Photo, got:', media?.constructor?.name);
      return undefined;
    }
    ...
    // eslint-disable-next-line no-console
    console.debug(`[DLPHOTO] photoId=${photo.id} dcId=${photo.dcId} sizeType="${args.sizeType}" sizes=[${photo.sizes.map((s) => ('type' in s ? s.type : '?')).join(',')}] picked=${size ? ('type' in size ? size.type : '?') : 'null'}`);
    ...
    if (!size
      ...
      // eslint-disable-next-line no-console
      console.warn(`[DLPHOTO] no usable size for photoId=${photo.id} sizeType="${args.sizeType}"`);
      return undefined;
    }
    ...
    // eslint-disable-next-line no-console
    console.debug(`[DLPHOTO] → downloadFile photoId=${photo.id} thumbSize="${size.type}" fileSize=${fileSize} dcId=${photo.dcId}`);
