...
export default function useMediaWithLoadProgress(
  ...
  const prevMediaHashRef = useRef(mediaHash);
  ...
  useEffect(() => {
    if (!noLoad && mediaHash) {
      if (!mediaData) {
        // If already loading the same hash (e.g. isSynced changed), don't cancel — let it finish
        if (startedAtRef.current && prevMediaHashRef.current === mediaHash) {
          return;
        }
        ...
        prevMediaHashRef.current = mediaHash;
