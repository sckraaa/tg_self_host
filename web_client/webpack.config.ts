...
const CSP = `
  default-src 'self';
  connect-src 'self' ws: wss: blob: http: https: ${APP_ENV === 'development' ? 'ipc:' : ''};
  script-src 'self' 'wasm-unsafe-eval' blob: https://t.me https://telegram.me;
...
export default function createConfig(
  ...
  return {
    ...
    devServer: {
      ...
      proxy: [
        {
          context: ['/apiws', '/apiws_test', '/apiws_premium', '/apiws_test_premium'],
          target: 'ws://localhost:8080',
          ws: true,
        },
        {
          context: ['/apiw1', '/apiw1_test', '/apiw1_premium', '/apiw1_test_premium'],
    ...
    plugins: [
      ...
      new EnvironmentPlugin({
        ...
        TELEGRAM_CUSTOM_SERVER: 'localhost',
        TELEGRAM_CUSTOM_PORT: '8080',
        TELEGRAM_CUSTOM_PROTOCOL: 'ws',
        // Branding & external URLs
        APP_TME_LINK_PREFIX: 'http://localhost:1234/',
        APP_FEEDBACK_URL: '',
        APP_FAQ_URL: '',
        APP_PRIVACY_URL: '',
...
function getGitMetadata() {
  try {
    ...
    return { branch: 'docker', commit: 'unknown' };
  }
}
