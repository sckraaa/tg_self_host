...
export function rsaDecrypt(buffer, privateKeyPem) {
    const key = createPrivateKey({ key: privateKeyPem, format: 'pem' });
    const decrypted = privateDecrypt({ key, padding: constants.RSA_NO_PADDING }, buffer);
    return decrypted;
}
...
export class IGE {
    ...
    encryptIge(data) {
        if (data.length % 16 !== 0)
            throw new Error('IGE: data length must be multiple of 16');
...
export function readBufferFromBigInt(value, length) {
    const hex = value.toString(16).padStart(length * 2, '0');
    return Buffer.from(hex, 'hex');
}
export function readBigIntFromBuffer(buffer, littleEndian = true) {
    if (littleEndian) {
        let result = 0n;
