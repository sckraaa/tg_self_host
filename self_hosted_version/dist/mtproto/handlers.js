...
function aggregateReactions(ownerUserId, peerKey, messageId, selfUserId) {
...
function inputPrivacyKeyToString(cid) {
    switch (cid) {
...
function parseInputPrivacyRule(reader, cid) {
    switch (cid) {
        ...
        case 0x840649cf: { // inputPrivacyValueAllowChatParticipants — chats:Vector<long>
            ...
            const chatIds = [];
            for (let i = 0; i < count; i++)
                chatIds.push(Number(reader.readLong()));
...
const _TG_DH_PRIME = Buffer.from('C71CAEB9C6B1C9048E6C522F70F13F73980D40238E3E21C14934D037563D930F' +
...
function _writePhoneCallProtocol(w) {
...
function _buildPhoneCallContainer(phoneCallBuf) {
...
function _buildPhoneCallWaiting(call, accessHash, includeReceiveDate) {
    const w = new BinaryWriter();
...
function _buildPhoneCallRequested(call, accessHash) {
    const w = new BinaryWriter();
...
function _buildPhoneCallAccepted(call, accessHash) {
    const w = new BinaryWriter();
...
function _buildPhoneConnectionWebrtc(id, ip, port, isTurn, isStun, username, password) {
    const w = new BinaryWriter();
...
function _buildConnectionsVector() {
    const w = new BinaryWriter();
...
function _buildPhoneCallActive(call, accessHash, gAOrB) {
    const w = new BinaryWriter();
...
function _buildPhoneCallDiscarded(id) {
    const w = new BinaryWriter();
...
function _buildUpdatePhoneCall(phoneCallBuf) {
    const w = new BinaryWriter();
...
function _readInputPhoneCall(r) {
...
function handleTlRequestImpl(data, session, reqMsgId, ctx) {
    ...
    switch (constructorId) {
        ...
        case 0xc1cd5ea9: { // initConnection
            ...
            if (innerQuery)
                return handleTlRequest(innerQuery, session, reqMsgId, ctx);
            return buildConfig();
        }
        ...
        case 0xa677244f: { // auth.sendCode
            ...
            if (phoneErr)
                return buildRpcErrorObject(400, phoneErr);
        ...
        case 0x8d52a951: { // auth.signIn
            ...
            if (codeErr)
                return buildRpcErrorObject(400, codeErr);
        ...
        case 0xaac7b717: { // auth.signUp
            ...
            if (fnErr)
                return buildRpcErrorObject(400, fnErr);
            ...
            for (const [, pendingCall] of _callsMap) {
                if (pendingCall.participantId === stateUserId && pendingCall.state === 'waiting') {
                    setTimeout(() => {
                        ...
                        const pendingEnvelope = buildLiveUpdatesEnvelope([pendingUpdateBuf], [String(pendingCall.adminId), String(pendingCall.participantId)], []);
        ...
        case 0xefea3803: { // langpack.getStrings
            ...
            for (let i = 0; i < keyCount; i++)
                keys.push(readTlString(lpReader2));
        ...
        case 0x3072cfa1: { // gzip_packed
            ...
            try {
                ...
                console.error(`[${new Date().toISOString()}] Session ${session.id} gzip decompress error:`, e.message);
                return null;
            }
        }
        ...
        case 0x29ee847a: { // messages.search
            ...
            if (searchReq) {
                ...
                if (results.length > 0) {
                    const fixtureMessages = results.map((msg) => ({
                        id: msg.messageId,
                        peerKey: msg.peerKey,
                        date: msg.date,
                        text: msg.text,
        ...
        case 0x545cd15a: { // messages.sendMessage
            ...
            if (peerErr)
                return buildRpcErrorObject(400, peerErr);
            ...
            if (isP2P) {
                ...
                let recipientReplyToMsgId;
                if (request.replyToMsgId) {
                    const [resolved] = messageStore.resolveRecipientMessageIds(selfId, request.peerKey, recipientId, recipientPeerKey, [request.replyToMsgId]);
            ...
            if (isGroupChat) {
                ...
                for (const p of participants) {
                    ...
                    if (pFixture) {
                        ...
                        ctx.broadcastToUser(p.userId, buildLiveUpdatesEnvelopeWithChats([buildUpdateNewMessage(pFixture, participantMsg.updatePts, participantMsg.updatePtsCount)], pUserIds, [chatId], p.userId));
                    }
                }
            }
            const senderUpdates = [
        ...
        case 0x51e842e1: { // messages.editMessage
            ...
            if (editPeerErr)
                return buildRpcErrorObject(400, editPeerErr);
            ...
            const editUserIds = [String(editSelfId)];
            const editChatIds = [];
            collectEntityIdsFromPeerKey(editFixture.peerKey, editUserIds, editChatIds);
            if (editFixture.fromPeerKey)
                collectEntityIdsFromPeerKey(editFixture.fromPeerKey, editUserIds, editChatIds);
        ...
        case 0xe58e95d2: { // messages.deleteMessages
            ...
            if (delIdsErr)
                return buildRpcErrorObject(400, delIdsErr);
            ...
            if (delReq.revoke) {
                ...
                if (isP2PDel) {
                    ...
                    recipientMsgIds = messageStore.resolveRecipientMessageIds(delSelfId, firstMsg.peerKey, recipientId, recipientPeerKey, delReq.messageIds);
        ...
        case 0xbe5335be: { // upload.getFile
            ...
            if (fileMimeType) {
                if (fileMimeType.includes('png'))
                    storageFileType = 0x0a4f63c0; // storage.filePng
                else if (fileMimeType.includes('gif'))
                    storageFileType = 0xcae1aadf; // storage.fileGif
                else if (fileMimeType.includes('mp4') || fileMimeType.includes('video'))
                    storageFileType = 0xb3cea0e4; // storage.fileMp4
                else if (fileMimeType.includes('webp'))
                    storageFileType = 0x1081464c; // storage.fileWebp
                else if (fileMimeType.includes('pdf'))
                    storageFileType = 0xae1e508d; // storage.filePdf
                else if (fileMimeType.includes('ogg') || fileMimeType.includes('audio') || fileMimeType.includes('opus'))
                    storageFileType = 0xaa963b05; // storage.fileUnknown (no specific type for audio)
            }
            const w = new BinaryWriter();
        ...
        case 0x330e77f: { // messages.sendMedia
            ...
            if (mediaReq.docAttributes) {
                for (const attr of mediaReq.docAttributes) {
                    if (attr.type === 'imageSize') {
                        mediaWidth = attr.w;
                        mediaHeight = attr.h;
                    }
                    if (attr.type === 'filename') {
                        mediaFileName = attr.name;
                    }
                    if (attr.type === 'audio') {
            ...
            if (mediaIsP2P) {
                ...
                let recipientMediaReplyToMsgId;
                if (mediaReq.replyToMsgId) {
                    const [resolved] = messageStore.resolveRecipientMessageIds(mediaSelfId, mediaReq.peerKey, mediaRecipientId, mediaRecipientPeerKey, [mediaReq.replyToMsgId]);
                ...
                if (recipientMediaFixture) {
                    ...
                    const recipientMediaUpdates = [
                        buildUpdateNewMessage(recipientMediaFixture, recipientMediaMsg.updatePts, recipientMediaMsg.updatePtsCount),
                    ];
                    ctx.broadcastToUser(mediaRecipientId, buildLiveUpdatesEnvelope(recipientMediaUpdates, recipientMediaUserIds, []));
                }
            }
            // Group/channel: broadcast to all other participants
            if (mediaIsGroupChat) {
                ...
                for (const p of mediaParticipants) {
                    ...
                    if (pMediaFixture) {
                        ...
                        ctx.broadcastToUser(p.userId, buildLiveUpdatesEnvelopeWithChats([buildUpdateNewMessage(pMediaFixture, participantMediaMsg.updatePts, participantMediaMsg.updatePtsCount)], pMediaUserIds, [mediaChatId], p.userId));
                    }
                }
            }
            const mediaSenderUpdates = [
        ...
        case 0x78515775: { // account.updateProfile
            ...
            let newAbout;
            if (profileFlags & (1 << 0))
                newFirstName = readTlString(reader);
            if (profileFlags & (1 << 1))
                newLastName = readTlString(reader);
            if (profileFlags & (1 << 2))
                newAbout = readTlString(reader);
        ...
        case 0x2714d86c: { // account.checkUsername
            ...
            if (validErr)
                return buildRpcErrorObject(400, validErr);
        ...
        case 0x13704a7c: { // messages.forwardMessages
            ...
            for (let i = 0; i < fwdReq.messageIds.length; i++) {
                ...
                if (fixture) {
                    ...
                    if (fwdFromPeerKey)
                        collectEntityIdsFromPeerKey(fwdFromPeerKey, userIds, chatIds);
                }
        ...
        case 0xd30d78d4: { // messages.sendReaction
            ...
            if (reactReq?.peerKey && reactReq.msgId) {
                ...
                if (reactReq.peerKey.startsWith('user:') && reactReq.peerKey !== `user:${reactSelfId}`) {
                    ...
                    const [otherMsgId] = messageStore.resolveRecipientMessageIds(reactSelfId, reactReq.peerKey, otherUserId, otherPeerKey, [reactReq.msgId]);
                    if (otherMsgId) {
                        if (reactReq.reactions.length > 0) {
        ...
        case 0x4bc6589a: { // messages.searchGlobal
            ...
            if (sgReq && sgReq.query.trim()) {
                ...
                if (sgReq.usersOnly)
                    peerPrefix = 'user:';
                else if (sgReq.groupsOnly)
                    peerPrefix = 'group:';
                else if (sgReq.broadcastsOnly)
                    peerPrefix = 'channel:';
        ...
        case 0x92ceddd4: { // messages.createChat
            ...
            for (const uid of req.userIds) {
                ...
                if (resolvedUid === selfId || resolvedUid === 0)
                    continue;
            ...
            for (const p of allParticipants) {
                ...
                if (pFixture) {
                    ...
                    ctx.broadcastToUser(p.userId, buildLiveUpdatesEnvelopeWithChats([buildUpdateNewMessage(pFixture, participantSvc.updatePts, participantSvc.updatePtsCount)], pUserIds, [chat.id], selfId));
                }
            }
            ...
            if (svcFixture) {
                ...
                for (const p of allParticipants) {
                    if (p.userId !== selfId)
                        creatorUserIds.push(String(p.userId));
                }
                ctx.broadcastSessionUpdates(session, buildLiveUpdatesEnvelopeWithChats([buildUpdateNewMessage(svcFixture, svcMsg.updatePts, svcMsg.updatePtsCount)], creatorUserIds, [chat.id], selfId));
            }
        ...
        case 0x35ddd674: { // messages.editChatPhoto
            ...
            for (const p of participants) {
                ...
                ctx.broadcastToUser(p.userId, buildLiveUpdatesEnvelopeWithChats([pUpdate], [String(selfId)], [req.chatId], selfId));
            }
            return buildLiveUpdatesEnvelopeWithChats([svcUpdate], [String(selfId)], [req.chatId], selfId);
        }
        case 0xa2b5a3f6: { // messages.getExportedChatInvites
            ...
            w.writeInt(0); // count
            w.writeInt(0x1cb5c415);
            w.writeInt(0); // empty vector invites
            w.writeInt(0x1cb5c415);
            w.writeInt(0); // empty vector users
            return w.getBytes();
        }
        case 0xdf04dd4e: { // messages.getChatInviteImporters
            ...
            w.writeInt(0); // count
            w.writeInt(0x1cb5c415);
            w.writeInt(0); // empty vector importers
            w.writeInt(0x1cb5c415);
            w.writeInt(0); // empty vector users
            return w.getBytes();
        }
        case 0x5bd0ee50: { // messages.deleteChat
            ...
            for (const p of participants) {
                ...
                deletionUpdates.set(p.userId, buildLiveUpdatesEnvelopeWithChats([update], [String(selfId)], [chatId], selfId, p.userId));
            }
        ...
        case 0x3bd2b4a0: {
            ...
            const call = _callsMap.get(_callKey(callId));
            if (!call)
                return buildRpcErrorObject(400, 'CALL_ALREADY_DECLINED');
            call.gb = gb;
            call.state = 'accepted';
        ...
        case 0xb2cbc1c0: {
            ...
            let reason = 'hangup';
            if (reasonConstructorRaw === 0x85e42301)
                reason = 'missed';
            else if (reasonConstructorRaw === 0xe095c1a0)
                reason = 'disconnect';
            else if (reasonConstructorRaw === 0xfabdef90)
                reason = 'busy';
            ...
            if (callerFixture) {
                ctx.broadcastToUser(call.adminId, buildLiveUpdatesEnvelope([buildUpdateNewMessage(callerFixture, callerMsg.updatePts, callerMsg.updatePtsCount)], [String(call.adminId), String(call.participantId)], []));
            }
            ...
            if (calleeFixture) {
                ctx.broadcastToUser(call.participantId, buildLiveUpdatesEnvelope([buildUpdateNewMessage(calleeFixture, calleeMsg.updatePts, calleeMsg.updatePtsCount)], [String(call.adminId), String(call.participantId)], []));
            }
