...
export function writeTlString(w, str) {
...
export function writeTlBytes(w, data) {
    let header;
    let headerLen;
    if (data.length <= 253) {
        header = Buffer.alloc(1);
...
export function readTlBytesRaw(reader) {
...
export function readTlString(reader) {
    const first = reader.readByte();
    let length = first;
    if (first === 254) {
        const bytes = reader.readBytes(3);
...
export function skipTlString(data, offset) {
...
export function skipJsonValue(data, offset) {
...
export function skipTlStringByReader(reader) {
    const first = reader.readByte();
    let length = first;
    if (first === 254) {
        const bytes = reader.readBytes(3);
...
export function skipInputPeer(reader) {
...
export function parseInputReplyTo(reader) {
    const constructorId = reader.readInt() >>> 0;
    switch (constructorId) {
...
export function skipTlVector(reader) {
...
export function skipTlObject(reader) {
...
// ========== Writer helpers ==========
export function writeEmptyVectorToWriter(w) {
    w.writeInt(0x1cb5c415);
    w.writeInt(0);
}
export function writeBufferVector(w, items) {
    w.writeInt(0x1cb5c415);
...
export function writeIntVector(w, values) {
    w.writeInt(0x1cb5c415);
...
export function writeEmptyJsonObject(w) {
    // jsonObject#99c1d49d value:Vector<JSONObjectValue>
    w.writeInt(0x99c1d49d);
    w.writeInt(0x1cb5c415);
    w.writeInt(0);
}
export function parseInitConnection(data) {
    try {
...
export function skipInitConnection(data) {
    ...
    try {
        ...
        console.log(`[${new Date().toISOString()}] Failed to parse initConnection:`, e.message);
        return null;
    }
}
//# sourceMappingURL=tlHelpers.js.map