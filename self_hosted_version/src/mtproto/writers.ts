 ...
 */
export function isFieldVisibleByPrivacy(targetUserId: number, viewerId: number, privacyKey: string): boolean {
  if (targetUserId === viewerId) return true; // always see own data
  const rules = messageStore.getPrivacyRules(targetUserId, privacyKey);
  if (rules.length === 0) return true; // default: visible
    ...
    switch (rule.ruleType) {
      ...
      case 'allowPremium':
        // Check if viewer is premium
        {
          ...
          if (viewerUser?.premium) return true;
        }
        break;
      case 'allowUsers':
        if (rule.value.includes(viewerId)) return true;
        break;
      case 'disallowUsers':
        if (rule.value.includes(viewerId)) return false;
        break;
...
export function writePeerByKey(w: BinaryWriter, peerKey: string): void {
...
export function writeMessageFromFixture(w: BinaryWriter, message: FixtureMessage): void {
  ...
  if (message.fwdFromPeerKey || message.fwdFromName) flags |= (1 << 2); // fwd_from
  if (message.replyToMsgId) flags |= (1 << 3);
  ...
  if (message.fwdFromPeerKey || message.fwdFromName) {
    ...
    let fwdFlags = 0;
    if (message.fwdFromPeerKey) fwdFlags |= (1 << 0);
    if (message.fwdFromName && !message.fwdFromPeerKey) fwdFlags |= (1 << 5);
    w.writeInt(fwdFlags);
...
function writeMessageReactions(w: BinaryWriter, reactions: Array<{ emoticon: string; count: number; chosenOrder?: number }>): void {
 ...
 */
function writeServiceMessageFromFixture(w: BinaryWriter, message: FixtureMessage): void {
...
function writeMessageAction(w: BinaryWriter, action: NonNullable<FixtureMessage['action']>): void {
  switch (action.type) {
    case 'chatCreate': {
    ...
    case 'phoneCall': {
      ...
      let mflags = 0;
      if (action.reason) mflags |= (1 << 0);
      if (action.duration !== undefined) mflags |= (1 << 1);
      if (action.isVideo) mflags |= (1 << 2);
      w.writeInt(mflags);
      w.writeLong(BigInt(action.callId || '0'));
      if (action.reason) {
        switch (action.reason) {
          case 'missed': w.writeInt(0x85e42301); break;
          case 'disconnect': w.writeInt(0xe095c1a0); break;
          case 'hangup': w.writeInt(0x57adc690); break;
          case 'busy': w.writeInt(0xfabdef90); break;
          default: w.writeInt(0x57adc690); break;
        }
      }
...
function writeMessageMedia(w: BinaryWriter, media: StoredMedia): void {
  if (media.type === 'photo') {
...
function writeMessageMediaPhoto(w: BinaryWriter, media: StoredMedia): void {
...
function writeMessageMediaDocument(w: BinaryWriter, media: StoredMedia): void {
  ...
  if (media.isVoice) mediaDocFlags |= (1 << 8); // voice
  ...
  if (media.isVoice || (media.duration !== undefined && media.mimeType?.startsWith('audio/'))) {
    attrs.push(() => {
      ...
      let audioFlags = 0;
      if (media.isVoice) audioFlags |= (1 << 10); // voice
      if (media.waveform) audioFlags |= (1 << 2); // waveform
...
export function writeUserFromFixture(w: BinaryWriter, user: FixtureUser, viewerId?: number): void {
  ...
  // --- flags ---
  let flags = 0;
  if (user.accessHash) flags |= (1 << 0);  // access_hash
  if (user.firstName) flags |= (1 << 1);    // first_name
  if (user.lastName) flags |= (1 << 2);     // last_name
  if (user.username) flags |= (1 << 3);     // username
  if (showPhone) flags |= (1 << 4);         // phone
  if (showPhoto) flags |= (1 << 5);         // photo
  flags |= (1 << 6);                        // status (always)
  if (user.bot) flags |= (1 << 14);         // bot
  if (user.bot) flags |= (1 << 15);         // bot_nochats (default false, bit always present for bots)
  if (user.verified) flags |= (1 << 17);    // verified
  if (user.restricted) flags |= (1 << 18);  // restricted
  if (user.scam) flags |= (1 << 24);        // scam
  ...
  if (user.emojiStatusDocumentId) flags |= (1 << 30); // emoji_status
  ...
  if (user.colorId !== undefined) flags2 |= (1 << 8); // color
  if (user.profileColorId !== undefined) flags2 |= (1 << 9); // profile_color
  if (user.botActiveUsers !== undefined) flags2 |= (1 << 11); // bot_active_users
  if (user.contactRequirePremium) flags2 |= (1 << 10); // contact_require_premium
  ...
  if (user.colorId !== undefined) {
    ...
    if (user.colorBackgroundEmojiId) colorFlags |= (1 << 1);
  ...
  if (user.profileColorId !== undefined) {
    ...
    w.writeInt(0xb54b5acf);
    let pcFlags = (1 << 0);
    if (user.profileColorBackgroundEmojiId) pcFlags |= (1 << 1);
    w.writeInt(pcFlags);
...
export function writeFallbackUserFromId(w: BinaryWriter, userId: string): void {
...
export function writeChatFromFixture(w: BinaryWriter, chat: FixtureChat): void {
...
export function writeChatFromDb(w: BinaryWriter, chat: StoredChat, selfId: number): void {
  const messageStore = getMessageStore();

  if (chat.type === 'channel' || chat.type === 'supergroup') {
    // channel#1c32b11c
    let flags = 0;
    if (chat.creatorUserId === selfId) flags |= (1 << 0); // creator
    if (chat.isBroadcast) flags |= (1 << 5); // broadcast
    if (chat.isMegagroup) flags |= (1 << 8); // megagroup
...
function writeChatPhoto(w: BinaryWriter, photoId: number | undefined, messageStore: ReturnType<typeof getMessageStore>): void {
  if (!photoId) {
...
export function writeChannelFromFixture(w: BinaryWriter, chat: FixtureChat): void {
  let flags = 0;
...
export function writeDialogFromFixture(w: BinaryWriter, dialog: FixtureDialog): void {
...
export function writeDialogVector(w: BinaryWriter, dialogs: FixtureDialog[]): void {
  w.writeInt(0x1cb5c415);
...
export function writeMessageVector(w: BinaryWriter, messages: FixtureMessage[]): void {
  w.writeInt(0x1cb5c415);
...
export function writeUserVector(w: BinaryWriter, fixture: OfficialCaptureFixture, userIds: string[]): void {
...
export function writeChatVector(w: BinaryWriter, fixture: OfficialCaptureFixture, chatIds: string[]): void {
...
export function writeDraftMessage(w: BinaryWriter, text: string, date: number, replyToMsgId?: number): void {
  ...
  let draftFlags = 0;
  if (replyToMsgId) draftFlags |= (1 << 4); // reply_to present
...
export function writeChatPhotoEmpty(w: BinaryWriter): void {
  w.writeInt(0x37c1011c);
}

export function writePhotoEmpty(w: BinaryWriter, photoId: bigint): void {
  // photoEmpty#2331b22d id:long
  w.writeInt(0x2331b22d);
  w.writeLong(photoId);
}

export function writeUpdatesStateToWriter(w: BinaryWriter, userId?: number): void {
...
export function writePeerColorSet(w: BinaryWriter, colors: number[]): void {
...
export function writePeerColorProfileSet(w: BinaryWriter, colors: number[]): void {
