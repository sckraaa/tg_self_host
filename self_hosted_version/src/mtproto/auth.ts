...
function readTlBytes(reader: BR): Buffer {
...
function getKeyByFingerprint(fingerprint: bigint): { n: bigint; e: number; fingerprint: bigint } | undefined {
...
export function generateObfuscatedHeader(): { header: Buffer; encryptor: CTR; decryptor: CTR } {
  let random: number[];
...
function generateKeyDataFromNonce(serverNonce: Buffer, newNonce: Buffer): { key: Buffer; iv: Buffer } {
...
function padToBlock(data: Buffer, blockSize = 16): Buffer {
...
export class AuthHandler {
  ...
  private authKeys: Map<string, Buffer> = new Map();
  private authKeysByKeyId: Map<string, Buffer> = new Map();
  ...
  clearAuthState(sessionId: string) {
    this.authState.delete(sessionId);
  }

  setAuthKey(sessionId: string, key: Buffer) {
  ...
  private computeAuthKeyId(key: Buffer): string {
    return sha1Sync(key).slice(12, 20).toString('hex');
  }

  getAllAuthKeys(): Array<{ keyIdHex: string; authKey: Buffer }> {
  ...
  processReqDHParamsSync(buffer: Buffer, sessionId: string): Buffer {
    const reader = new BR(buffer);
    const constructorId = reader.readInt(false);
    if (constructorId !== 0xd712e4be) throw new Error("Invalid req DH params constructor");
