...
export function startServer(port: number, host: string) {
  ...
  function handleDisconnect(session: ClientSession) {
  ...
  function processSessionData(buffer: Buffer, session: ClientSession) {
  ...
  const tcpServer = net.createServer((socket) => {
    ...
    socket.on('data', (data: Buffer) => {
      console.log(`[${new Date().toISOString()}] Session ${clientId} TCP data: len=${data.length}, first16=${data.slice(0, 16).toString('hex')}`);
      session.lastActivityAt = Date.now();
...
function processBuffer(buffer: Buffer, session: ClientSession, preDecrypted = false): ProcessResult | null {
  ...
  if (!session.obfuscated && !session.handshakeComplete) {
    ...
    console.log(`[${new Date().toISOString()}] Session ${session.id} obfuscation OK, remaining=${buffer.length - 64} bytes`);
  ...
  msgLen = msgLen << 2;
  console.log(`[${new Date().toISOString()}] Session ${session.id} frame: firstByte=0x${decrypted[0].toString(16)}, headerLen=${headerLen}, msgLen=${msgLen}, bufLen=${decrypted.length}, preDecrypted=${preDecrypted}`);
  ...
  console.log(`[${new Date().toISOString()}] Session ${session.id} authKeyId=${authKeyId}, msgData.len=${msgData.length}, remaining=${remaining.length}`);
...
function handleUnencryptedAuthPayload(buffer: Buffer, session: ClientSession): Buffer | null {
  ...
  console.log(`[${new Date().toISOString()}] Session ${session.id} auth payload: 0x${constructorId.toString(16)}, len=${buffer.length}`);
